...
const Feedback = () => {
  ...
  return (
    <>
      <div class=" bg-blue-500 hover:bg-blue-600 text-white fixed bottom-4 right-4 z-50 font-bold py-2 px-4 rounded-full focus:outline-none" onClick={toggleOpen}>
      ...
      <div class={`${open? "opacity-100" : "opacity-0"} transition-opacity duration-200 flex flex-col justify-center fixed w-11/12 bottom-20 sm:bottom-20 sm:right-20 bg-white z-50 px-6 py-7 rounded-md bg-gray-600 shadow-lg sm:w-72`}>
        {submitted? (
          <div class={`h-64 flex flex-col justify-between`}>
            ...
            <p class="text-white font-semibold text-lg">Thank you for submitting your question!</p>
            <p class="text-white">We will reach out to you shortly.</p>
          </div>
        ) : (
          <div>
          ...
          <form onSubmit={onSubmit} class="space-y-10 sm:space-y-4">
            ...
            <div class="space-y-2">
              <label for="rating" class="block text-sm font-medium text-white">Couldn't find your question ?</label>
              
            </div>
            <div class="space-y-2">
              <label for="message" class="block text-sm font-medium text-white">Write your question here: </label>
              <div class="mt-1">
